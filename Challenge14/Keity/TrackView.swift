...
class TrackView: UIView {
    ...
    private var capaHeightConstraint: NSLayoutConstraint!
    private var infoRowTopConstraint: NSLayoutConstraint!
    private var albumImageWidthConstraint: NSLayoutConstraint?
...
//        capaHeightConstraint = capa.heightAnchor.constraint(equalTo: capa.widthAnchor)
        infoRowTopConstraint = infoRow.topAnchor.constraint(equalTo: capa.bottomAnchor, constant: 24)

        capa.translatesAutoresizingMaskIntoConstraints = false

        let widthConstraint = capa.widthAnchor.constraint(equalToConstant: 0) //ancora largura
        albumImageWidthConstraint = widthConstraint
        ...
        NSLayoutConstraint.activate([
            ...
            capa.topAnchor.constraint(equalTo: safeAreaLayoutGuide.topAnchor, constant: 120),
...
//            lyrics.bottomAnchor.constraint(lessThanOrEqualTo: safeAreaLayoutGuide.bottomAnchor, constant: -16),
    ...
    override func layoutSubviews() {
        ...
        updateAlbumImageLayout()
    }
    private func updateAlbumImageLayout() {
            let isLandscape = bounds.width > bounds.height
            let availableWidth = max(bounds.width - 64, 0)
            let imageWidth = isLandscape ? availableWidth * 0.15 : availableWidth

            albumImageWidthConstraint?.constant = imageWidth
        }

    func updateForSize(_ size: CGSize) {
        let isLandscape = size.width > size.height
        print("📐 updateForSize chamado — size: \(size), isLandscape: \(isLandscape)")

        capa.isHidden = isLandscape

        
        infoRowTopConstraint.isActive = false
        capaHeightConstraint.isActive = false

        if isLandscape {
            infoRowTopConstraint = infoRow.topAnchor.constraint(equalTo: navBar.bottomAnchor, constant: 16)
        } else {
            infoRowTopConstraint = infoRow.topAnchor.constraint(equalTo: capa.bottomAnchor, constant: 24)
            capaHeightConstraint.isActive = true
        }

        infoRowTopConstraint.isActive = true
        layoutIfNeeded()
    }
